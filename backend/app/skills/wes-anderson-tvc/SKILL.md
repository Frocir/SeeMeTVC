...
1. **问清**（可与用户已给的 Brief 合并，缺什么问什么）：品牌、产品、一个卖点、时长、画幅、禁忌。未齐时不要 `run_image_to_video`。
...
4. **搭图连线**：`add_node` / `connect` / `patch_node`。拓扑与必接端口以节点规约为准（系统提示里的卡片），不要另造一套链。
5. **生成**：先跑免费/LLM/模拟；扣费的图生视频必须等用户确认卡，不要连催。
